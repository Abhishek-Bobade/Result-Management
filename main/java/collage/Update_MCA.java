...
@WebServlet("/upmca")
public class Update_MCA extends GenericServlet {
	Connection con;
		public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException{	
		try {
			con = DataBase.getCon();
			String s1 = req.getParameter("roll"); 
			PreparedStatement ptsm = con.prepareStatement("select * from MCA_STUDENT where ROLL_NO=?");
			ptsm.setString(1, s1);
			...
			if (rs.next()) {
				...
				pw.println("<form action='upmarksmca' method='post'>");
				pw.println("<input id='i2' type='text' placeholder='Java Programming' name='java' value="+rs.getString(6)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='Data Structure' name='dsa' value="+rs.getString(7)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='OOSE' name='oose' value="+rs.getString(8)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='Operating System' name='os' value="+rs.getString(9)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='Networking' name='nt' value="+rs.getString(10)+" autocomplete='off' required> 	<br></br></br></br> ");
