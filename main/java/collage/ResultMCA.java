...
@WebServlet("/resultmca")
public class ResultMCA extends GenericServlet {
	public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
		try {
			Connection con = DataBase.getCon();
			PreparedStatement pstm = con.prepareStatement("select * from MCA_STUDENT ORDER BY Roll_No");
			ResultSet rs = pstm.executeQuery();
			PrintWriter pw = res.getWriter();
			pw.println("<html><head><title>MCA Result</title>");
			pw.println("<style> td{ width:100px;}#td1{width:250px;}</style>");
			pw.println("</head><body bgcolor=#6d756f text=black>");
			pw.println("<center><h1>" + "Student Details of MCA" + "</h1><h1 style=color:white;>Dashboard</h1></center><hr><br>");
