...
@WebServlet("/upbca")
public class Update_BCA extends GenericServlet {
	Connection con;
		public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException{	
		try {
			con = DataBase.getCon();
			String s1 = req.getParameter("roll"); 
			PreparedStatement ptsm = con.prepareStatement("select * from BCA_STUDENT where ROLL_NO=?");
			ptsm.setString(1, s1);
			...
			if (rs.next()) {
				...
				pw.println("<form action='upmarksbca' method='post'>");
				pw.println("<input id='i2' type='text' placeholder='Java Programming' name='java' value="+rs.getString(6)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='Python Programming' name='python' value="+rs.getString(7)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='eCommerce' name='eCommerce' value="+rs.getString(8)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='Software Engineering' name='se' value="+rs.getString(9)+" autocomplete='off' required> "
						+ "<input id='i2' type='text' placeholder='Artificial Intiligence' name='ai' value="+rs.getString(10)+" autocomplete='off' required> 	<br></br></br></br> ");
